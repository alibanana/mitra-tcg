...
export async function CtaSection({
  ...
  const { instagram, whatsapp } = await settingsService.getSocialLinks()
  const instagramUrl = instagram || siteConfig.instagram
  const whatsappUrl = whatsapp || siteConfig.whatsapp || ""
  const showSocialButtons = !ctaHref;

  return (
    <section className="site-cta py-20">
      <div className="container mx-auto px-6 text-center lg:px-12">
        <p className="site-cta-overline theme-tagline text-xs">Mitra TCG</p>
        <h2 className="site-cta-heading mt-4 text-4xl font-bold md:text-5xl">
          {heading}
        </h2>
        <p className="site-cta-body mx-auto mt-4 max-w-xl text-base">{body}</p>
        <div className="mt-8 flex flex-col items-center justify-center gap-5 sm:flex-row sm:gap-3">
          {showSocialButtons ? (
            <>
              <Link
                href={instagramUrl}
